...
function identList() {
   ...
   d.vars = [];
   d.firstVals = [];
   d.vars.push(identifier());
   if (tok == ASSIGN) d.firstVals.push(withValue());
   else d.firstVals.push(0);
   while (tok == COMMA) {
      match(COMMA);
      d.vars.push(identifier());
      if (tok == ASSIGN) d.firstVals.push(withValue());
...
function statList() {
   let L = [];
   let dec = met.list[0];
   for (let i = 0; i < dec.vars.length; i++) { // decleration vals added as assigment
      let a = new Assignment(dec.vars[i], dec.firstVals[i]);
      L.push(a);
   }
   do { //at least one
      let f = tok.index;
